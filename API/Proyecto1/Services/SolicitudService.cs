...
namespace Proyecto1.Services
{
    public class SolicitudService
    {
        ...
        public List<Solicitud> GetAllSolicitudesEstudiante(string Estudiante)
        {
            ...
            command = new SqlCommand("select ES.Descripcion,S.IdCarnet,S.IdSolicitud, TB.IdTipoBeca, TB.Nombre, S.IdEstado  from Solicitud as S inner join estudiantes as E on E.carne = S.IdCarnet inner join EstudiantexFormulario as EF on E.carne = EF.IdCarnet inner join Formulario as F on EF.IdFormulario = F.IdFormulario inner join TipoBeca as TB on TB.IdTipoBeca = F.IdTipoBeca  inner join EstadoSolicitud as ES on ES.IdEstado = S.IdEstado where S.[Delete] = 0 and S.IdCarnet =" + Estudiante.ToString(), conn);
            read = command.ExecuteReader();
            while (read.Read())
            {
                Solicitud sol = new Solicitud();
